...
import { getPlotListByGroupChatId } from '../../../services/ai/group-chat'
...
Page({
  ...
  onLoad(options) {
    ...
    if (options.groupId) {
      ...
      this.loadPlotList(options.groupId)
   ...
   * 加载剧情列表
   */
  loadPlotList(groupChatId) {
    const params = {
      groupChatId: groupChatId || this.data.groupInfo.id
    }
  ...
  async onPlotTap(event) {
    ...
    try {
      await setCurrentPlot({ plotId: id, groupChatId: this.data.groupInfo.id })
      Toast({
        context: this,
        selector: '#t-toast',
        message: '剧情切换成功',
      ...
      wx.navigateTo({
        url: `/pages/chat/index?plotId=${id}&groupId=${this.data.groupInfo.id}`
      })
    } catch (error) {
